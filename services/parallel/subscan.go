...
func SubscanEvents(moduleId, eventId string, startBlock int64, page int64) (list []SubscanEvent) {
	...
	url := fmt.Sprintf("%s/api/scan/events", config.Cfg.SubscanHost)
	p := SubscanParams{
		Row:       100,
		Page:      page,
		Call:      eventId,
	...
	if err != nil {
		logrus.Errorf("post subscan event failed", "err", err)
		return nil
	}
...
func SubscanExtrinsic(extrinsicIndex string) *ExtrinsicDetail {
	...
	url := fmt.Sprintf("%s/api/scan/extrinsic", config.Cfg.SubscanHost)
...
func SubscanLogs(blockNum uint) []SubscanLog {
	...
	url := fmt.Sprintf("%s/api/scan/logs", config.Cfg.SubscanHost)
...
func SubscanBlockHeader(blockNum uint) *BlockHeader {
	...
	url := fmt.Sprintf("%s/api/scan/header", config.Cfg.SubscanHost)
	raw, err := PostWithApiKey(url, strings.NewReader(fmt.Sprintf(`{"block_num":%d}`, blockNum)))
	if err != nil {
		return nil
	}
